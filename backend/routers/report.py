...
async def export_pdf_report_endpoint(payload: PdfExportPayload, background_tasks: BackgroundTasks):
    ...
    try:
        logger.info("📄 Iniciando exportação de relatório PDF para a sessão: %s no idioma: '%s'", payload.job_id, payload.language)
        ...
        logger.info("✅ Relatório PDF para a sessão %s gerado e pronto para download.", payload.job_id)
        return FileResponse(
            str(pdf_path),
        ...
        logger.error("❌ Erro Interno em /report/pdf_export para a sessão %s: %s", payload.job_id, e, exc_info=True)
        if DEBUG:
