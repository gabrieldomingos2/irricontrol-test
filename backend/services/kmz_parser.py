...
) -> Tuple[Optional[AntenaData], List[PivoData], List[CicloData], List[BombaData]]:
    """
    Processa um arquivo KMZ, extrai dados de antena, pivôs, ciclos e bombas.
    Retorna: Tupla contendo (dados_antena, lista_pivos, lista_ciclos, lista_bombas).
    """
    caminho_kmz = Path(caminho_kmz_str)
    pasta_extracao = Path(pasta_extracao_str)

    antena_data: Optional[AntenaData] = None
        ...
        for placemark_node in root.findall(".//kml:Placemark", KML_NAMESPACE):
            ...
            if geometry_type == "Point" and coordenadas_placemark:
                ...
                if any(keyword in nome_normalizado for keyword in ANTENA_KEYWORDS):
                    ...
                    logger.info(f"  -> Antena encontrada: {nome_original} ({lat:.6f}, {lon:.6f}) Alt: {altura}m")
            ...
            elif geometry_type == "LineString" and len(coordenadas_placemark) >= 3:
                if eh_um_circulo(coordenadas_placemark):
                    ciclos_list.append({
                        "nome_original_circulo": nome_original,
                        "coordenadas": coordenadas_placemark
                    })
                    logger.info(f"  -> 🔵 Ciclo (de LineString) detectado: {nome_original}")
                else:
                    logger.info(f"  -> ⚠️ LineString ignorada (não parece círculo): {nome_original}")
            
            elif geometry_type == "Polygon" and len(coordenadas_placemark) >= 3:
                ciclos_list.append({
                    "nome_original_circulo": nome_original,
                    "coordenadas": coordenadas_placemark
                })
                logger.info(f"  -> 🟦 Ciclo (de Polygon) detectado: {nome_original}")
        
        pivos_finais_list = _consolidate_pivos(pivos_de_pontos_list, ciclos_list, pontas_retas_map)

        logger.info(f"✅ Processamento KMZ concluído: {len(pivos_finais_list)} pivôs, {len(bombas_list)} bombas.")
        return antena_data, pivos_finais_list, ciclos_list, bombas_list
