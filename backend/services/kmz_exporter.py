...
import json
import re
# NOVO: Importa o módulo datetime para gerar os nomes únicos
...
def _setup_main_antenna_structure(
    ...
    # Esta função não precisa de alterações, apenas recebe o novo nome da subpasta
...
def _add_overlays_and_repeater_structures(
    ...
    template_id: str, # ALTERAÇÃO: Recebe o ID do template para gerar o nome
    ...
    # ... (código do ground_main da antena principal sem alterações) ...
        ...
        if nome_imagem_rep.lower().startswith("repetidora_") and nome_imagem_rep.lower().endswith(".png"):
            ...
            if json_rep_path_servidor.exists():
                ...
                if bounds_rep_data:
                    ...
                    # 1. GERAÇÃO DE NOME ÚNICO PARA A SUBPASTA DA REPETIDORA
                    now_rep = datetime.now()
                    # Adiciona microssegundos para garantir nome único mesmo em loop rápido
...
def _add_secondary_folders(doc: simplekml.Document, pivos: list, ciclos: list, bombas: list, default_point_style: simplekml.Style):
    # Esta função não precisa de alterações
    if pivos: #... (código omitido, continua o mesmo)
        pass
    if ciclos: #... (código omitido, continua o mesmo)
        pass
    if bombas: #... (código omitido, continua o mesmo)
        pass
...
def build_kml_document_and_get_image_list(
    ...
    study_date_str_for_subfolder: str, # Este não será mais usado, mas mantemos por consistência
    ...
    image_files_for_kmz = _add_overlays_and_repeater_structures(
        ...
        colour_key_filename,
        main_coverage_name, 
        template_id=template_id_for_subfolder, # Passa o ID do template
