 ...
 */
function getExportKmzUrl(jobId, imagem, boundsFile, repetidorasData = []) {
    if (!jobId) {
        console.error("Erro fatal: Job ID é necessário para exportar o KMZ.");
        mostrarMensagem("Erro: ID da Sessão não encontrado para exportação.", "erro");
        return "#"; // Retorna uma URL inválida para evitar uma chamada quebrada.
    }

    // 👇 ALTERADO: A URL agora inclui o jobId como um parâmetro de query obrigatório.
    let url = `${BACKEND_URL}${API_PREFIX}/kmz/exportar?job_id=${jobId}&imagem=${encodeURIComponent(imagem)}&bounds_file=${encodeURIComponent(boundsFile)}`;
